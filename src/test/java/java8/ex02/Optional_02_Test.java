...
public class Optional_02_Test {
    ...
    public void test_optional_map() throws Exception {
        ...
        Optional<Integer> julesAge = julesOpt.map(p -> {return p.getAge();});
    ...
    public void test_optional_ifPresent() throws Exception {
        ...
        // TODO déclencher l'exception GoodException pour valider que la fonction en paramètre de ifPresent a bien été exécutée.
         Optional<Integer> julesAge = julesOpt.filter(adult).map(Person::getAge);
         assertThat(julesAge.isPresent(), is(true));
         assertThat(julesAge.get(), is(30));
         julesAge.ifPresent(p ->{ throw new GoodException();});
