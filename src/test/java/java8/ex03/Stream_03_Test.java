...
public class Stream_03_Test {
    ...
    public void test_mapping() throws Exception {
        ...
        List<Customer> customers = new Data().getCustomers();
        Comparator<Customer> comparator = (a,b) -> a.getFirstname().compareTo(b.getFirstname());

        // TODO Construire la map Sexe -> Chaîne représentant les prénoms des clients
        Map<Gender, String> result =customers.stream().sorted(comparator).collect(
                Collectors.groupingBy(Customer::getGender, Collectors.mapping(Customer::getFirstname, Collectors.joining("|"))));
